...
<html lang="en"><!--<![endif]--> 
	<head>
		...
		function agree($tid){
					...
					//$tid=$("#tid").val();
					$.post('__APP__/Home/Solve/add',{tid:$tid},function(json){
				...
				function refuse($tid){
				   ...
				   $.post('__APP__/Home/Solve/delete',{tid:$tid},function(json){
				   });
						document.getElementById($tid).innerHTML="";
			...
			<div class="row">
				...
				<div class="span9">
					...
					<table class="table table-bordered table-striped">
						...
						<?php if(is_array($title)): $i = 0; $__LIST__ = $title;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tbody id="<?php echo ($vo["tid"]); ?>">
							...
							<tr>
								...
								<td>
								<?php echo ($vo["tdepartclass"]); ?>
								</td>
								<td>
								<?php echo ($vo["tphone"]); ?>
								</td>
								<td>
									<a href="__APP__/Home/Detailed/index/tid/<?php echo ($vo["tid"]); ?>" class="__APP__/Home/Files/index">View</a>
								</td>
								<td>
								<button type="button" class="btn btn-primary" id="<?php echo ($vo["tid"]); ?>" onclick="agree(<?php echo ($vo["tid"]); ?>)">同意</button>
								&nbsp;&nbsp;
&nbsp;									<button type="button" class="btn btn-danger"id="<?php echo ($vo["tid"]); ?>" onclick="refuse(<?php echo ($vo["tid"]); ?>)">拒绝</button>
								</td>
							</tr>
